...
//! - V2: `session.context` pushes the session repository baseline onto the
//!   outgoing root-Lead request's system context at every model dispatch —
//!   an ephemeral injection that is never persisted into the user message or
//!   the session history;
...
// The latest user message text is the session's current task signal for the
// bridge (it drives task-reset bookkeeping only). The repository baseline
// itself stays session/repository scoped and task-independent.
  ...
  setup: async (ctx) => {
    ...
    registrations.push(await ctx.session.hook("context", async (event) => {
      ...
      const result = await bridge("session.context", {
        ...
        agent: event.agent,
        text: lastUserText(event.messages),
      });
...
"#,
    ...
    fn v2_source_uses_a_node_spawn_and_never_rewrites_the_request() {
        ...
        // session `context` hook, never by mutating the admitted prompt.
...
// Prompt admission is not an OCG surface anymore: the adapter must not
// register a prompt hook at all, so the persisted user message stays whatever
// the user typed.
...
"#;
    ...
    fn v2_context_hook_supplies_one_baseline_per_lead_dispatch() {
        ...
        // The admitted prompt is never an OCG surface.
        assert_eq!(value["promptHookRegistered"], json!(false));
        ...
        assert_eq!(value["afterNextTurn"], json!(3));
        ...
        assert_eq!(value["totalLines"], json!(4));

        // Exact direct argv, inherited environment and payloads.
        ...
        // The continuation sends the unchanged latest user text; the next turn
        // sends the new one; the separate session reports its own id.
        let stdin: Value = serde_json::from_str(records[1]["stdin"].as_str().unwrap()).unwrap();
        assert_eq!(stdin["text"], json!("hello lead"));
        let stdin: Value = serde_json::from_str(records[2]["stdin"].as_str().unwrap()).unwrap();
        assert_eq!(stdin["text"], json!("second turn"));
        let stdin: Value = serde_json::from_str(records[3]["stdin"].as_str().unwrap()).unwrap();
        assert_eq!(stdin["session_id"], json!("s4"));
        assert_eq!(stdin["agent"], json!("lead-high"));
    }
