...
//                       -> report the raw text of a *completed* root Lead
//                          assistant message to the bridge, which persists
//                          `.opencode-gear/reports/latest-lead-output.md`
//                          (no headers, no summary, byte-verbatim text)
//
...
// Only a *completed* root Lead assistant message is captured: the assistant
// role, an OCG Lead agent, no error (an interrupted/aborted message carries
// one) and a completion timestamp. A message without `time.completed` is still
// streaming and is never reported.
function completedLeadMessage(event) {
  if (!event || event.type !== "message.updated") return null;
  const data = event.data || {};
  const info = data.info;
  if (!info || info.role !== "assistant") return null;
  if (!isLead(info.agent)) return null;
  if (info.error) return null;
  if (!info.time || typeof info.time.completed !== "number") return null;
  if (typeof info.id !== "string" || !info.id) return null;
  return { sessionID: data.sessionID, messageID: info.id, agent: info.agent };
}

// Durable text parts of the messages that are still streaming. A part is
// replaced in place on every update, so the last state wins; first-seen order
// preserves the order of the user-visible text parts.
function recordTextPart(store, event) {
  ...
  parts.set(part.id, {
    order: existing ? existing.order : parts.size,
    text: part.text,
    hidden: part.synthetic === true || part.ignored === true,
  });
  // Bound the memory of a long session: parts of messages that never complete
  // (for example an interrupted turn) are eventually dropped.
  while (store.size > 64) {
...
function rawLeadText(store, messageID) {
  const parts = store.get(messageID);
  if (!parts) return "";
  return Array.from(parts.values())
    .filter((part) => !part.hidden && part.text.trim().length > 0)
    .sort((left, right) => left.order - right.order)
    .map((part) => part.text)
    .join("\n\n");
}

// Report exactly one completed root Lead message, or remember a streaming text
// part. The adapter never writes and never summarizes: the raw text goes to the
// Rust bridge, which re-checks the policy and writes atomically.
async function captureLeadOutput(event, store) {
  const completed = completedLeadMessage(event);
  if (!completed) {
    recordTextPart(store, event);
    return;
  }
  const text = rawLeadText(store, completed.messageID);
  store.delete(completed.messageID);
  if (!text) return;
  await bridge("lead.output", {
...
export default {
  ...
  setup: async (ctx) => {
    ...
    // the other registrations. A disabled switch, or a runtime context without
    // the event surface, registers nothing at all.
    if (reportsLatestEnabled() && ctx.event && typeof ctx.event.subscribe === "function") {
      let stream = null;
      try {
        ...
        const pump = (async () => {
          try {
            for await (const event of stream) {
              if (stopped) break;
              try {
                await captureLeadOutput(event, textParts);
              } catch (_) {
...
function makeCtx(stream) {
  ...
  return {
    ...
    ctx: {
      event: stream ? { subscribe: () => stream.iterator } : undefined,
...
const textPart = (sessionID, messageID, partID, text, extra = {}) => ({
  type: "message.part.updated",
  data: { sessionID, part: { id: partID, messageID, type: "text", text, ...extra } },
});
const reasoningPart = (sessionID, messageID, partID) => ({
  type: "message.part.updated",
  data: { sessionID, part: { id: partID, messageID, type: "reasoning", text: "thinking" } },
});
const message = (sessionID, id, agent, extra = {}) => ({
  type: "message.updated",
  data: { sessionID, info: { id, role: "assistant", agent, time: { created: 1, completed: 2 }, ...extra } },
});
const streaming = (sessionID, id, agent) => ({
  type: "message.updated",
  data: { sessionID, info: { id, role: "assistant", agent, time: { created: 1 } } },
});
...
"#;
    ...
    #[test]
    fn v2_lead_output_reports_only_completed_root_lead_messages() {
        if !node_or_skip("v2_lead_output_reports_only_completed_root_lead_messages") {
            return;
        }
        let dir = tempfile::tempdir().unwrap();
        write_v2_plugin(&dir);
...
// 1. Streaming: text parts exist but the message is not completed yet, and a
//    reasoning part is never user-visible text.
stream.push(textPart("s1", "m1", "p1", "first part"));
stream.push(reasoningPart("s1", "m1", "r1"));
stream.push(streaming("s1", "m1", "lead-high"));

// 2. Completed root Lead message: ordered text parts (one updated twice), a
//    synthetic part that must be excluded, and the completion event.
stream.push(textPart("s1", "m1", "p2", "second part"));
stream.push(textPart("s1", "m1", "p1", "first part, final"));
stream.push(textPart("s1", "m1", "p3", "hidden", {{ synthetic: true }}));
stream.push(message("s1", "m1", "lead-high"));

// 3. A worker child session and a non-OCG agent never report.
stream.push(textPart("s2", "m2", "p1", "worker text"));
stream.push(message("s2", "m2", "ocg-build"));
stream.push(textPart("s3", "m3", "p1", "other text"));
stream.push(message("s3", "m3", "general"));

// 4. An interrupted/errored message never replaces the last completed output;
//    a resumed completion does.
stream.push(textPart("s4", "m4", "p1", "interrupted text"));
stream.push(message("s4", "m4", "lead-low", {{ error: {{ name: "MessageAbortedError" }} }}));
stream.push(textPart("s4", "m5", "p1", "resumed final text"));
stream.push(message("s4", "m5", "lead-low"));
...
"#
        ...
        assert_eq!(
            all.len(),
            2,
            "only the two completed Lead messages: {value}"
        );
        for entry in all {
            let argv = entry["argv"].as_array().unwrap();
            assert_eq!(argv[1], json!("lead.output"), "{entry}");
        }
        let first: Value = serde_json::from_str(all[0]["stdin"].as_str().unwrap()).unwrap();
        assert_eq!(first["session_id"], json!("s1"));
        assert_eq!(first["agent"], json!("lead-high"));
        assert_eq!(first["message_id"], json!("m1"));
        assert_eq!(
            first["text"],
            json!("first part, final\n\nsecond part"),
            "ordered user-visible text parts only: {first}"
        );
        let second: Value = serde_json::from_str(all[1]["stdin"].as_str().unwrap()).unwrap();
        assert_eq!(second["session_id"], json!("s4"));
        assert_eq!(second["agent"], json!("lead-low"));
        assert_eq!(second["text"], json!("resumed final text"));
    }
    ...
    fn v2_lead_output_is_inert_when_disabled_or_without_the_event_surface() {
        ...
        write_check(
            ...
            &format!(
                r#"{LEAD_OUTPUT_PRELUDE}
// The switch is off: a completed Lead message must not reach the bridge.
const stream = makeStream();
const first = makeCtx(stream);
await (await plugin.setup(first.ctx))();
stream.push(textPart("s1", "m1", "p1", "must not be reported"));
stream.push(message("s1", "m1", "lead-high"));
