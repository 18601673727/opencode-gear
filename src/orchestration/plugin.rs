...
// prompt in the same session gets an empty `context`, which this adapter treats
// as a no-op, so the snapshot is not duplicated across turns. A materially
// changed snapshot is injected again on the next prompt and becomes the new
// session baseline.
//
...
export default {
  ...
  setup: async (ctx) => {
    ...
    registrations.push(await ctx.session.hook("prompt", async (event) => {
      ...
      const context = contextWithMetadata(result);
      if (context) event.prompt.text = text + suffix(context);
    }));
...
"#;
    ...
    #[test]
    fn v2_prompt_presents_estimated_metadata_and_no_ops_on_cached_context() {
        if !node_or_skip("v2_prompt_presents_estimated_metadata") {
            return;
        }
        #[cfg(unix)]
        {
...
// A fresh snapshot: the adapter presents a compact, clearly-estimated header
// before the detailed context and never claims exact provider tokens.
process.env.OCG_FIXTURE_MODE = "metadata";
const fresh = { sessionID: "s1", prompt: { text: "hello lead" } };
await hooks["prompt"](fresh);

// A cached snapshot: the bridge returns an empty context, which is a no-op.
process.env.OCG_FIXTURE_MODE = "cached";
const cached = { sessionID: "s1", prompt: { text: "hello again" } };
await hooks["prompt"](cached);

console.log(JSON.stringify({
  fresh: fresh.prompt.text,
  cached: cached.prompt.text,
  count: lines(),
}));
"#;
            write_check(&dir, &format!("{JS_PRELUDE}{body}"));
            let value = run_check(&dir, &bridge, &record);
            let fresh = value["fresh"].as_str().unwrap();
            assert!(fresh.starts_with("hello lead"), "{fresh}");
            assert!(fresh.contains("fixture snapshot body"));
            assert!(fresh.contains(CONTEXT_START) && fresh.contains(CONTEXT_END));
            // Compact, estimated metadata: `≈6.7k tokens`, never "exact".
            assert!(
                fresh.contains("OCG Context · ≈6.7k tokens · 23 files · 117 symbols"),
                "{fresh}"
            );
            // An empty (cached) context must not append anything.
            assert_eq!(value["cached"], json!("hello again"));
            assert_eq!(value["count"], json!(2));
        }
    }
