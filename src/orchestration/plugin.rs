...
"#
        ...
        assert_eq!(
            all.len(),
            4,
            "two output writes and two ordered context observations: {value}"
        );
        ...
        assert_eq!(
            contexts.len(),
            2,
            "each completed output gets one context boundary: {value}"
        );
        let first: Value = serde_json::from_str(outputs[0]["stdin"].as_str().unwrap()).unwrap();
        ...
        let second: Value = serde_json::from_str(outputs[1]["stdin"].as_str().unwrap()).unwrap();
