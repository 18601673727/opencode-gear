...
//                          response step to the bridge, which persists
...
// replace a previously captured good output. The step's agent comes from its
// own `session.step.started`, falling back to the session-level selection;
// without a positive Lead identification nothing is reported.
...
// Handle exactly one event: remember agent ownership and streaming text, and
// report one completed root Lead step. The adapter never writes and never
// summarizes: the raw text goes to the Rust bridge, which re-checks the policy
...
// 3. A worker child session and a non-OCG agent never report, even with a
//    completed stop step.
...
// 4. A step whose `session.step.started` was missed falls back to the
//    session-level agent selection; an unidentified step never reports.
...
"#
        ...
        assert_eq!(
            all.len(),
            2,
            "only the two completed Lead responses: {value}"
        );
        for entry in all {
        ...
        assert_eq!(first["message_id"], json!("m2"));
        assert_eq!(
            first["text"],
            json!("first part, final\n\nsecond part"),
            "ordered text ordinals only, ended text preferred over deltas: {first}"
        );
