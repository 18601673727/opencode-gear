...
CREATE TABLE IF NOT EXISTS runs (
 ...
 generation INTEGER NOT NULL CHECK(generation > 0), contract TEXT NOT NULL,
...
impl SubstrateRepository {
    ...
    pub fn load(&mut self, mission: &MissionId) -> Result<Option<MissionState>> {
        ...
        let mut stmt = tx.prepare("SELECT run_id,node_id,generation,contract,state,created_at,finished_at,result FROM runs WHERE mission_id=?1 ORDER BY run_id").map_err(sql)?;
        let rows = stmt
            .query_map([mission.as_str()], |r| {
                Ok((
                    r.get::<_, i64>(0)?,
            ...
            .map_err(sql)?;
        for row in rows {
            let (key, node, generation, contract, status, created_at, finished_at, result) =
                row.map_err(sql)?;
    ...
    pub fn create_child(
        ...
        event(
            ...
            mission,
            "child_created",
            &json(&serde_json::json!({"node_id":next,"parent_node_id":parent.0}))?,
            Some(by),
        )?;
        tx.commit().map_err(sql)?;
        Ok(WorkNodeId(next))
    ...
    pub fn start_run(
        ...
        insert_run(&tx, mission, next, node, generation, &contract, now)?;
    ...
    pub fn replace_run(
        ...
        let tx = self.conn.transaction().map_err(sql)?;
        let (_, generation, active, _) = node_witness(&tx, mission, node)?;
        if active != Some(old.0) {
        ...
        let new_generation = generation
            .checked_add(1)
            .ok_or_else(|| invalid("generation overflow"))?;
        tx.execute("UPDATE runs SET state='fenced',finished_at=?4 WHERE mission_id=?1 AND run_id=?2 AND state='active' AND generation=?3",params![mission.as_str(),num(old.0)?,generation,now]).map_err(sql)?;
        insert_run(&tx, mission, next, node, new_generation, &contract, now)?;
        tx.execute("UPDATE work_nodes SET generation=?3,active_run_id=?4 WHERE mission_id=?1 AND node_id=?2",params![mission.as_str(),num(node.0)?,new_generation,num(next)?]).map_err(sql)?;
        event(
            &tx,
            mission,
            "run_replaced",
...
fn insert_run(
    ...
    tx.execute(
        "INSERT INTO runs VALUES (?1,?2,?3,?4,?5,'active',?6,NULL,NULL)",
        params![
            mission.as_str(),
            num(run)?,
            num(node.0)?,
